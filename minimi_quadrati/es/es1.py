...
import funzioni_Approssimazione_MQ as fz
...
n = int(input("Inserisci grado del polinomio "))
a = fz.metodoQR(x,y,n)
# Genero dei dati da valutare
xval = np.linspace(np.min(x), np.max(x), 100)
# Valuta il polinomio
p = np.polyval(a, xval)
plt.plot(xval, p, "-r", x,y , "bo")
plt.legend(["Polinomio di grado "+ str(n), "Dati del problema"])